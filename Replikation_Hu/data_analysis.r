...
data_phase2 %>%
  ...
  labs(
    x = "Block",
    y = "Display-Bearbeitungszeit in s"
  ) + 
  scale_x_discrete(
    labels = c("Geringer Zeitdruck", "Mittlerer Zeitdruck", "Hoher Zeitdruck*")
  )
  ggsave("plot.pdf", width = 15, height = 10, units = "cm", dpi = 300)
  ggsave("plot1.pdf", dpi = 300)
...
ggplot(data_phase1, aes(reaction_time, colour = block)) +
  ...
  scale_colour_manual(
    ...
    labels = c(
      ...
      "low time pressure" = "Geringer Zeitdruck",
      "medium time pressure" = "Mittlerer Zeitdruck",
      "high time pressure" = "Hoher Zeitdruck")
  ) +
  theme(aspect.ratio = 0.6) +
  labs(
    x = "Display-Bearbeitungszeit in s",
    y = "Kumulative Wahrscheinlichkeit"
  ) +
  labs(colour = "Block")

ggsave("plot2.pdf", width = 15, height = 10, units = "cm", dpi = 300)
ggsave("plot3.pdf", dpi = 300)
